...
class App {
    ...
    static public function exec() {
        ...
        try{
	        ...
	        try{
		        $first = new \ReflectionParameter(array($module, $action), 0);
            ...
            if($method->isPublic() && !$method->isStatic()) {
                ...
                if($method->getNumberOfParameters()>0 && C('URL_PARAMS_BIND')){
	                ...
	                if(isset($first) && $first->name == 'request'){
    ...
    static public function run() {
        #edit by qzw 2015-06-15
        // 加载动态应用公共文件和配置
        load_ext_file(COMMON_PATH);
        $isAjax = isset($_SERVER['HTTP_X_REQUESTED_WITH']) && $_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest';
        $setupXhprof = (C('APP_XHPROF') && !$isAjax || $isAjax && C('APP_XHPROF_AJAX') || intval($_REQUEST['xhprof'])) && function_exists('xhprof_enable');
        // 应用初始化标签
