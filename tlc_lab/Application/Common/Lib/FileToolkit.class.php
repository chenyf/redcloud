...
class FileToolkit
{
        ...
        if(is_dir($path)){
            ...
            if($handle){
                while(($file = readdir($handle)) !== false){
                    ...
                    $item["nametruth"] = $filename;
                    $item["abs"] = $file;

                    $info = pathinfo($file);
                    $ext = $info['extension'];
                    if(mb_strlen($filename) > 40){
                        $item["namebrief"] = mb_substr($filename,0,30) . "***" ."." . $ext;
                    }else{
                        $item["namebrief"] = $filename;
                    }
    ...
    public static function validateIsFileType(File $file){
        if(!empty($file)){
    ...
    {
        return $file instanceof UploadedFile ? $file->getClientSize() : 0;
    }
    

    public static function getSecureFileExtensions()
    {
        return 'jpg jpeg gif png txt doc docx xls xlsx pdf ppt pptx pps ods odp mp4 mp3 webm ogg avi flv wmv wma mov zip rar gz tar 7z swf ico';
    }
    ...
    //删除文件;只有老师才可能删除文件
    public static function deleteFile($user,$filepath){
        if($user->isTeacher()) {
    ...
    public static function statFile($filepath){
        ...
        $result['data']['ext'] = self::getFileExtension($file);
        $result['data']['filesize'] = filesize($filepath);
    ...
    public static function isVideoNeedToBeConvert($extension){
        $extension = strtolower($extension);
        if (self::getFileTypeByExtension($extension) != 'video') {
            return false;
        }

        $ok = ['mp4', 'ogg', 'webm'];
        return !in_array($extension, $ok); //不在ok数组里面的需要转换
    }
    ...
    {   
        ...
        if (in_array($extension, array('mp4', 'avi', 'wmv', 'flv', 'mov', 'ogg', 'webm', '3gp'))) {
            return 'video';
