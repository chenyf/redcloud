    ...
    var VideoChooser = BaseChooser.extend({
    	...
    	attrs: {
    		uploaderSettings: {
                ...
                file_types : "*.mp4;*.webm;*.ogg;*.mov;*.wmv;*.flv;*.rmvb;*.avi;*.3gp",
        ...
        onChanged: function(item) {
            if(item.videoInfo){
                $('input[name=polyvVideoSize]').val(item.polyvVideoSize);
                $('input[name=minute]').val(item.minute);
                $('input[name=second]').val(item.second);
                $('input[name=polyvVid]').val(item.vid);
                $('input[name=media]').val(item.videoInfo);
            }

            this.element.find('[data-role=placeholder]').text(item.name);
            this.close();
        },
