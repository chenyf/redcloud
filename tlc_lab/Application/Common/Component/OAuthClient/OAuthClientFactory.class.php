<?php
namespace  Common\Component\OAuthClient;
...
class OAuthClientFactory{
    ...
    public static function create($type, array $config){
    	if (!array_key_exists('key', $config) or !array_key_exists('secret', $config)) {
            E('参数$config中，必需包含key, secret两个为key的值');
    	}
    ...
    public static function clients(){
        $clients = array(
            'weibo' => array(
                'name' => '微博帐号',
                'class' => 'Common\Component\OAuthClient\WeiboOAuthClient',
            ...
            'qq' => array(
                'name' => 'QQ帐号',
                'class' => 'Common\Component\OAuthClient\QqOAuthClient',
            ...
            'renren' => array(
                'name' => '人人帐号',
                'class' => 'Common\Component\OAuthClient\RenrenOAuthClient',
...
//        $kernel = ServiceKernel::instance();
        if (getParameter('oauth2_clients')) {
            $extras = getParameter('oauth2_clients');
