...
class EduCloudController extends \Home\Controller\BaseController
{
     ...
     */
    public function sendSmsCodeAction(Request $request){
        
         if($request->getMethod() == 'POST'){
            $type =  trim($request->request->get('sms_type'));
            $currentUser = $this->getCurrentUser();
            $uid = $currentUser['id'];
            $mobile = $request->request->get('to');
            if($type=="sms_forget_password"){
                $userInfo = createService("User.UserServiceModel")->getUserByVerifiedMobile($mobile);
                $uid = $userInfo['id'];
            }
            $mobileCodeService = $this->getMobileCodeService();
            $res = $mobileCodeService->sendMobileCode($mobile,$uid,$type);
            if($res){
                return $this->createJsonResponse(array('ACK' => 'ok','success' => '验证码已发送请注意查收'));
            }else{
                return $this->createJsonResponse(array('error' =>$mobileCodeService->getErrorMsg()));
            }
         }
    ...
    public function sendEmailCodeAction(Request $request){
         ...
         if($request->getMethod() == 'POST'){
            ...
            if($type=="sms_forget_password"){
                $userInfo = createService("User.UserServiceModel")->getUserByEmail($email);
                $uid = $userInfo['id'];
            }
            $emailCodeService = $this->getEmailCodeService();
            $res = $emailCodeService->sendEmailCode($email,$uid,$type);
            if($res){
                return $this->createJsonResponse(array('ACK' => 'ok','success' => '验证码已发送请注意查收'));
            }else{
                return $this->createJsonResponse(array('error' =>$emailCodeService->getErrorMsg()));
            }
         }
     ...
     * @author fubaosheng 2015-04-28
     */
    public function sendSmsPwdAction(Request $request){
        
         if($request->getMethod() == 'POST'){
            $mobile = $request->request->get('to');
            $code = strtolower($request->request->get('code')); 
            if(!isValidMobile($mobile))
                return $this->createJsonResponse(array('error' =>'手机号码格式不正确'));
            $userInfo = createService("User.UserServiceModel")->getUserByAccount($mobile);
            if(empty($userInfo))
                return $this->createJsonResponse(array('error' =>'此手机号没有注册过账号'));
            if(session('captcha_code') != $code)
                return $this->createJsonResponse(array('error' =>'验证码错误'));
            $uid = $userInfo['id'];
            $mobileCodeService = $this->getMobileCodeService();
            $res = $mobileCodeService->sendMobilePwd($mobile,$uid);
            if($res){
                return $this->createJsonResponse(array('ACK' => 'ok','success' => '密码已发送请注意查收'));
            }else{
                return $this->createJsonResponse(array('error' =>$mobileCodeService->getErrorMsg()));
            }
         }
     ...
     * @author fubaosheng 2015-04-28
     */
    public function smsCodeCheckAction(Request $request){
        $type = trim($request->query->get('type'));
        $code = $request->query->get('value');
        $mobile = $request->query->get('mobile');
        $mobileCodeService = $this->getMobileCodeService();
        $res = $mobileCodeService->checkMobileCode($mobile,$code,$type);
        if($res){
            $response = array('success' => true, 'message' => '');
        }else{
            $response = array('success' => false, 'message' => $mobileCodeService->getErrorMsg());
        }
     ...
     * @author fubaosheng 2015-04-28
     */
    public function smsPwdCheckAction(Request $request){
        $code = $request->query->get('value');
        $mobile = $request->query->get('mobile');
        $mobileCodeService = $this->getMobileCodeService();
        $res = $mobileCodeService->checkMobilePwd($mobile,$code);
        if($res){
            $response = array('success' => true, 'message' => '');
        }else{
            $response = array('success' => false, 'message' => $mobileCodeService->getErrorMsg());
     ...
     * @author fubaosheng 2015-04-28
     */
    public function emailCodeCheckAction(Request $request){
        $type = trim($request->query->get('type'));
        $code = $request->query->get('value');
        $email = $request->query->get('email');
        $emailCodeService = $this->getEmailCodeService();
        $res = $emailCodeService->checkEmailCode($email,$code,$type);
        if($res){
            $response = array('success' => true, 'message' => '');
        }else{
            $response = array('success' => false, 'message' => $emailCodeService->getErrorMsg());
        }
        return $this->createJsonResponse($response);
    }
