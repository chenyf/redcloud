...
class SmsToolkit
{
     ...
     * @param array $param        	
     * @return boolean 
     * @author yangjinlong 2015-04-29
     */
    public static function smsToCheck($request, $uid, $idcode){
        
        $verifyCode = M("user_idcode")->where("uid = {$uid} and idcode = {$idcode}")->find();
        if(!$verifyCode){
            return false;
        }else{
  //        M("user_idcode")->where("uid = {$uid} and idcode = {$idcode}")->delete();
            return true;  
        }
