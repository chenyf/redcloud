...
class CloudFileImplementorModel extends BaseModel
{   
...
//        $uploadFile['canDownload'] = empty($uploadFile['canDownload']) ? 0 : 1;
    ...
    {
        $params = $this->getCloudClient()->makeUploadParams($options);
        return $params;
    }
    ...
    private function getVideoWatermarkImages()
    {
        ...
        $videoWatermarkImage = $this->getEnvVariable('baseUrl').$this->getKernel()->getParameter('redcloud.upload.public_url_path')."/".$setting['video_embed_watermark_image'];
    ...
    {
        $diskDirectory = $this->getKernel()->getParameter('redcloud.disk.local_directory');
    ...
    private function getConvertor($name)
    {
        ...
        return new $class($this->getCloudClient(), getParameter('cloud_convertor'));
    }
...
class HLSEncryptedVideoConvertor extends HLSVideoConvertor
{
    public function getCovertParams($params)
    {
        ...
        $params['hlsKeyUrl'] = 'http://hlskey.redcloud.net/placeholder';
