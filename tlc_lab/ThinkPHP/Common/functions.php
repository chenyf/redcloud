...
function E($msg, $code=0) {
    // by qzw 2015-05-11
    if(!C('jsonException')) throw new Think\Exception($msg, $code);
    else{
        $param = array(
            'code'      => 500,
            'message'   => $msg,
            'secret'    => C('jsonException')=='noSecret' ? 0 : 1,
            'securityCode' => C('jsonExceptionSecurityCode'),
        );
        mobileJson($param);
    }
}
...
function U($url='',$vars='',$suffix=true,$domain=false) {
    ...
    if(C('URL_MODEL') == 0) { // 普通模式URL转换
        ...
        if($suffix) {
            ...
            if($suffix && '/' != substr($url,-1) && (substr($url,-4) != C('URL_HTML_SUFFIX')) ){
                $url  .=  '.'.ltrim($suffix,'.');
            }
        }
    }
...
function session($name='',$value='') {
    ...
    if(is_array($name)) { // session初始化 在session_start 之前调用
        ...
        if(''===$name){
            ...
            if($prefix) {
                doLog("prefix:" . $prefix);
                doLog("session-prefix:" . json_encode($_SESSION[$prefix]));
                unset($_SESSION[$prefix]);
            }else{
                doLog("sessions:" . json_encode($_SESSION));
                $_SESSION = array();
            }
...
function cookie($name='', $value='', $option=null) {
    ...
    $config = array(
        ...
        'domain'    =>  C('COOKIE_FULLE_DOMAIN'), // cookie 有效域名
   ...
   // $name = $config['prefix'] . str_replace('.', '_', $name);
    if ('' === $value) {
        ...
        if (is_null($value)) {
            ...
            doLog("这次有人设置cookie:name:" . $name ." - value:".$value);
            // 设置cookie
            if(is_array($value)){
...
//            doLog("设置cookie：".$name." : ".$value." : ".$expire ." : ".$config['path']. " : ".$config['domain']);
