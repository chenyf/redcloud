...
class LoginBindController extends \Home\Controller\BaseController
{
    ...
    public function chooseAction(Request $request, $type)
    {
        ...
        return $this->render('Login:bind-choose', array(
            'oauthUser' => $oauthUser,
            'type' => $type,
    ...
    private function generateUser($type, $token, $oauthUser,$setData)
    {
        ...
        if (!empty($setData['nickname']) && !empty($setData['email'])) {
            ...
            $registration['email'] = 'u_' . substr($randString, 0, 12) . '@redcloud.net';
        }
