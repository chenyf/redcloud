...
class ConvertVideoQueue {
	 ...
	 * @param array $param
	 */
	public function videoConvert($param=array()) {
		$options = array(
			'fileId' =>0,
			'isCenter'=>0,
		);
		...
		if(!file_exists(C('FFMPEG_CMD'))){
			self::getUploadFile()->setFileConvert($fileId, 'error');
			self::throwError(C('FFMPEG_CMD').'找不到命令，请程序确认是否安装 ',$file);
		}

		if($file['storage'] == 'local'){
			$path_part = pathinfo($filePath);
			$fileNewName = $path_part['filename'].'.mp4';
			$newPath = $path_part['dirname'].'/'.$fileNewName;
			$shell  = C('FFMPEG_CMD').' -i ' . $filePath . " " . $newPath;
		}else{
			$fileBaseName = basename($file['hashId']);
			$fileNewName = mb_substr($fileBaseName,0,mb_strrpos($fileBaseName,$file['ext'])) . "mp4";
		...
		if(!file_exists($newPath)){
			self::getUploadFile()->setFileConvert($fileId, 'error');
			//@unlink($filePath);
			self::throwError($filePath.' 视频转码失败->错误信息： '.var_export($err_info,true).' -> 错误码：'.$err_code);
		}
	...
	private function recordLog($log) {
		self::getLogService()->error("Queue",'ConvertVideoQueue.videoConvert',$log);
	}
