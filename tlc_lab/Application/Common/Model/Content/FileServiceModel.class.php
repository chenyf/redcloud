...
class FileServiceModel extends BaseModel
{
	...
	public function uploadFile($group, File $file, $target = null)
	{
        ...
        public function uploadDataFile($group, File $file, $target = null)
	{
		$errors = FileToolkit::validateFileExtension($file);
		if ($errors) {
			@unlink($file->getRealPath());
		...
		$record['uri'] = $this->generateDataUri($group, $file);
		$record['createdTime'] = time();
		$record = $this->getFileDao()->addFile($record);
		$record['file'] = $this->saveFile($file, $record['uri']);
	...
	private function saveFile($file, $uri)
	{
		...
		if ($parsed['access'] == 'public') {
			$directory = getParameter('redcloud.upload.public_directory');
		} else {
			$directory = getParameter('redcloud.upload.private_directory');
		}

		if (!is_writable($directory)) {
			E("文件上传路径{$directory}不可写，文件上传失败。");
		}
    ...
    private function generateDataUri ($group, $file)
    {
	    if ($file instanceof UploadedFile) {
		    $filename = $file->getClientOriginalName();
	    } else {
    ...
    {
        return $this->createDao('Content.FileModel');
    }

	private function getGroupDao()
	{
        return $this->createDao('Content.FileGroupModel');
	}
