define(function(require, exports, module) {
	...
	exports.run = function() {
		...
		$table.on('click','.check-group,.ban-group,.allow-group',function(){
			var $trigger = $(this);
			if (!confirm('是否要修改加入限制为:'+$trigger.text()+' ?')) {
				return ;
			}
			$.post($(this).data('url'), function(){
				Notify.success('加入状态:['+$trigger.text()+ ']修改成功！');
				setTimeout(function(){
					window.location.reload();
				},400);
			}).error(function(){
				Notify.danger('加入状态:['+$trigger.text()+ ']修改失败！');
			});
