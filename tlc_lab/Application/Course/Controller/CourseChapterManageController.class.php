<?php
namespace Course\Controller;

use Symfony\Component\HttpFoundation\Request;
use Common\Lib\ArrayToolkit;

class CourseChapterManageController extends \Home\Controller\BaseController
{

    public function createAction(Request $request, $id)
    {
        $course = $this->getCourseService()->tryManageCourse($id);
        $type = $request->query->get('type');
        $parentId = $request->query->get('parentId');
        $type = in_array($type, array('chapter', 'unit')) ? $type : 'chapter';
        if($request->getMethod() == 'POST'){
            $chapter = $request->request->all();

            $chapter['courseId'] = $course['id'];
            $chapter['parentId'] = empty($chapter['pid']) ? 0 : intval(str_replace("chapter-","",$chapter['pid']));
            unset($chapter['pid']);
            $chapter = $this->getCourseService()->createChapter($chapter);
            $default = $this->getSettingService()->get('default', array());
            return $this->render('CourseChapterManage:list-item', array(
                 'course' => $course,
                 'chapter' => $chapter,
                 'default'=> $default
            ));
        }
        
        $default = $this->getSettingService()->get('default', array());
        return $this->render('CourseChapterManage:chapter-modal', array(
            'course' => $course,
            'type' => $type,
            'parentId' => $parentId,
            'default'=> $default
        ));
    }

    public function editAction(Request $request, $courseId, $chapterId)
    {
        $course = $this->getCourseService()->tryManageCourse($courseId);
        $chapter = $this->getCourseService()->getChapter($courseId, $chapterId);
        if (empty($chapter)) {
            return $this->createMessageResponse('error',"章节(#{$chapterId})不存在！");
        }

        if($request->getMethod() == 'POST'){
            $fields = $request->request->all();
            $fields['courseId'] = $course['id'];
            $chapter = $this->getCourseService()->updateChapter($courseId, $chapterId, $fields);
            return $this->render('CourseChapterManage:list-item', array(
                'course' => $course,
                'chapter' => $chapter,
            ));
        }
        
        $default = $this->getSettingService()->get('default', array());
        return $this->render('CourseChapterManage:chapter-modal', array(
            'course' => $course,
            'chapter' => $chapter,
            'type' => $chapter['type'],
             'default'=> $default
        ));
        
    }

    public function deleteAction(Request $request, $courseId, $chapterId)
    {
        $course = $this->getCourseService()->tryManageCourse($courseId);
        //删除章及其下所有节
        $this->getCourseService()->deleteChapterAndLessons($course['id'], $chapterId);
//        $this->getCourseService()->deleteChapter($course['id'], $chapterId);

        return $this->createJsonResponse(true);
    }

    private function getCourseService()
    {
        return createService('Course.CourseService');
    }

        private function getSettingService()
    {
        return createService('System.SettingService');
    }
}