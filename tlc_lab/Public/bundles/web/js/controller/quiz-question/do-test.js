...
// 做试卷
        ...
        $('#testpaper-finish-btn').on('click', function() {
            ...
            if (isAjaxing == 0) {
                $.post($finishBtn.data('url'), {data: changeAnswers}, function() {
        ...
        if ($('#teacherCheckForm').length > 0) {
            ...
            Validator.addRule('score', function(options) {
                ...
                if (parseFloat(element.val()) <= parseFloat(element.data('score'))) {
                    return true;
                } else {
                    return false;
                }
        ...
        $('a.toggle_anwser').on('click',function(){
            var anwserBlock = $(this).next('div.anwser-and-analysis');
            anwserBlock.slideToggle();
            if($(this).text() == '收起答案'){
                $(this).text('查看答案');
            }else{
                $(this).text('收起答案');
    ...
    function formatTime(time) {
        ...
        var hour = parseInt(min / 60),
                mins = min - hour * 60;
        return (min > 0 ? pad(hour, 2) : "00") + ":" + (min > 0 ? pad(mins, 2) : "00") + ":" + pad(sec, 2);
    }
    ;
