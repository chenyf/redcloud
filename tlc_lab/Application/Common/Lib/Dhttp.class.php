...
{//类定义开始
	//通过get方式获取数据
	public function doGet($url,$timeout=5)
	{
	...
	//通过POST方式发送数据
	 public function doPost($url,$data,$timeout=5)
	{
		$code=self::getSupport();
		switch($code)
		{
	 ...
	 public function getSupport()
	{
	 ...
	 public function GetHttpContent($fsock=null) {
		$out = null;
	 ...
	 public function socketGet($url,$timeout=5){
		$url2 = parse_url($url);
	 ...
	 public function socketPost($url,$post_data=array(),$timeout=5){
		$url2 = parse_url($url);
