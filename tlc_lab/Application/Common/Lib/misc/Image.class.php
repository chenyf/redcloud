...
//        @imagestring($im, 5, 5, 3, $randval, $stringColor);
     ...
     * @param int $imageW 验证码宽度
     * @param int $imageH 验证码宽度
     * @param int $fontSize 字体大小
     * @param bool $useCurve 是否画混淆曲线
     * @param bool $useNoise  是否添加杂点
     */
    static function buildSecurityCode($length = 4, $imageW = 0, $imageH = 0, $fontSize = 14, $useCurve = true, $useNoise = true){

        $codeSet = '2346789ABCDEFGHJKLMNPQRTUVWXY';#验证码中使用的字符，01IO容易混淆，建议不用
        $imageW || $imageW = $length*$fontSize*1.5 + $fontSize*1.5; #图片宽(px)
        $imageH || $imageH = $fontSize * 2;#图片高(px)
        $_image = imagecreate($imageW, $imageH); #创建图像
        imagecolorallocate($_image, 243, 251, 254); #设置背景      
        #验证码字体随机颜色
        ...
        if ($useNoise) {
            ...
            for($i = 0; $i < 10; $i++){
                ...
                for($j = 0; $j < 5; $j++) {
                    imagestring($_image,5, mt_rand(-10, $imageW), mt_rand(-10, $imageH), $codeSet[mt_rand(0, 27)],$noiseColor);
                }
            }
        } 
        #绘干扰线
        ...
        for ($i = 0; $i<$length; $i++) {
            $code = $codeSet[mt_rand(0, 27)];
            $codeStr .= $code;
            $codeNX += mt_rand($fontSize*1.2, $fontSize*1.6);
            // 写一个验证码字符
            imagettftext($_image, $fontSize, mt_rand(-40, 70), $codeNX, $fontSize*1.5, $_color,SITE_PATH."/Application/Common/Lib/misc/simhei.ttf", $code);
        }
        session('captcha_code',strtolower($codeStr));
//        $_SESSION['verify']= md5(strtolower($codeStr));
        Image::output($_image, 'png');
