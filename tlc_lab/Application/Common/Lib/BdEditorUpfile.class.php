...
//        "POST" => "文件大小超出 post_max_size 限制" ,
    ...
    {
        $imgSizeArr = getimagesize($this->fullName);

        return array(
            "originalName" => $this->oriName ,
            "name" => $this->fileName ,
            "relUrl" => '/'. $this->date . '/'.$this->fileName,
            "size" => $this->fileSize ,
            'nowSize' => filesize($this->fullName),
            "type" => $this->fileType ,
            "state" => $this->stateInfo,
            'width' => $imgSizeArr[0],
            'height' => $imgSizeArr[1],
            'debug' => $_SERVER['SERVER_ADDR'].':'.$_SERVER['SERVER_PORT'].'_'.$_SERVER['HOSTNAME'].'/'.  posix_getpid()
        );
    }
