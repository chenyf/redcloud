...
use Common\Model\CloudPlatform\Client\WyzcAppClient;
use Common\Lib\ArrayToolkit;
class AppServiceModel extends BaseModel
{
    ...
    public function checkDependsForPackageUpdate($packageId)
    {
        ...
        try {
            $package = $this->getCenterPackageInfo($packageId);
            if (!version_compare(System::VERSION, $package['redcloudMinVersion'], '>=')) {
                $errors[] = "redcloud版本需大于等于{$package['redcloudMinVersion']}，您的版本为" . System::VERSION . '，请先升级redcloud';
            }
        } catch(\Exception $e) {
            $errors[] = $e->getMessage();
        }
    ...
    public function downloadPackageForUpdate($packageId)
    {
        ...
        try {
            $package = $this->getCenterPackageInfo($packageId);
            if (empty($package)) {
                E("应用包#{$packageId}不存在或网络超时，读取包信息失败");
            }
    ...
    private function _execScriptForPackageUpdate($package, $packageDir, $type)
    {
        ...
        $upgrade = new \WyzcUpgrade($this->getKernel());
    ...
    {
        return $this->getKernel()->getParameter('redcloud.disk.update_dir');
    }

    private function getBackUpDirectory()
    {
        return $this->getKernel()->getParameter('redcloud.disk.backup_dir');
    }
    ...
    private function hasWyzcMainApp($apps)
    {
        foreach ($apps as $app) {
            if($app['code'] === 'MAIN') {
                return true;
    ...
    private function addWyzcMainApp()
    {
        $app = array(
            'code' => 'MAIN',
            'name' => 'redcloud主系统',
            'description' => 'redcloud主系统',
            'icon' => '',
            'version' => 0,//System::VERSION, //qzw 2015-08-25
            'fromVersion' => '0.0.0',
            'developerId' => 1,
            'developerName' => 'redcloud官方',
            'installedTime' => time(),
            'updatedTime' => time(),
        );
    ...
    {
        return $this->createDao('CloudPlatform.CloudAppModel');
    }

    private function getAppLogDao ()
    {
        return $this->createDao('CloudPlatform.CloudAppLogModel');
    }
    ...
    private function createAppClient()
    {
        if (!isset($this->client)) {
            ...
            $this->client = new WyzcAppClient($options);
        }
        return $this->client;
    }
