...
class WeiXin
{
	...
	public function __construct($options)
	{
		...
		$this->_token = isset($options['token'])?$options['token']:$this->_token;
	 ...
	 * @return bool
	 */
	public function saveCookie($content){
		return S($this->_cookiename,$content,$this->_cookieexpired);
	}
	...
	public function verify_code() {
		...
		if ($result) {
			...
			if(count($matches)>1) {
				...
				if ($status==201) $_SESSION['weixin_weixin_login_step'] = 1;
				if ($status==200) {
