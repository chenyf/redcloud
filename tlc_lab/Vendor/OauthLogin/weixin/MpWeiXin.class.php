 ...
 *  
 */
require_once SITE_PATH . '/addons/libs/Snoopy.class.php';
class MpWeiXin
{
	private $cookie;
	private $_cookiename;
	...
	private $_account;
	private $_password;
	private $_datapath = 'weixin_cookie_';
	private $debug;
	private $_logcallback;
	private $_token;
	private $_accountConf;

	public function __construct($options)
	{
		$this->_accountConf = $this->getAccountConf();
		$this->_account = $this->_accountConf[$options['treeid']]['account'];
		$this->_password = $this->_accountConf[$options['treeid']]['password'];
	...
	/**
         ...
         * @return array 
         */
	private function getAccountConf(){

		$arrConf = require_once SITE_PATH .'/addons/plugins/login/weixin/config.php';
		return $arrConf;
	}
	...
	public function getMsg($lastid=0,$offset=0,$perpage=50,$day=0,$today=0,$star=0){
		...
		$send_snoopy->rawheaders['User-Agent'] = 'Mozilla/5.0 (Windows NT 5.2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/28.0.1500.72 Safari/537.36';
		$send_snoopy->referer = "https://mp.weixin.qq.com/cgi-bin/getmessage?t=wxm-message&lang=zh_CN&count=50&token=".$this->_token;
		$lastid = $lastid===0 ? '':$lastid;
	...
	public function getNewMsg(){
		...
		$send_snoopy->rawheaders['Cookie']= $this->cookie;
		$send_snoopy->rawheaders['Host'] = 'mp.weixin.qq.com';
		$url = $send_snoopy->referer = $send_snoopy->rawheaders['Referer'] = "https://mp.weixin.qq.com/cgi-bin/message?t=message/list&token=".$this->_token."&count=100&day=7";
		$send_snoopy->rawheaders['User-Agent'] = 'Mozilla/5.0 (Windows NT 5.2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/28.0.1500.72 Safari/537.36';
		$send_snoopy->fetch($url);
		preg_match("/list : \((.*?)\)\.msg_item/i",$send_snoopy->results,$rs);
	 ...
	 * @return fakeid 
	 */
	public function getFakeIdByMsg($msg = ''){
		$list = $this->getNewMsg();
		if (empty($list)) {
			return false;
		}
	...
	public function login(){
		...
		$snoopy->rawheaders['User-Agent'] = 'Mozilla/5.0 (Windows NT 5.2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/28.0.1500.72 Safari/537.36';
	...
	public function saveCookie($filename,$content){
		$filename = $filename ? $filename : $this->_cookiename;
		return S($filename,$content);
	}
