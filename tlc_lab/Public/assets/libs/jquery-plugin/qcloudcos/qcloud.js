function CosCloud(appid, signUrl){
	this.appid = appid;
	this.sign_url = signUrl || "/System/QcloudCos/AuthAction";
}
CosCloud.prototype.qcloudXHR = "";
...
//终止上传，@author fbs
CosCloud.prototype.killUpload = function(){
    if(!this.qcloudXHR) return false;
    try{
        var r = this.qcloudXHR.abort();
        console.info(r);
    }catch(e){
        console.info('kill xhr');
    }
}

CosCloud.prototype.sliceUploadFile = function(success, error, bucketName, remotePath, file, processCallBack){
	var that = this;
