    ...
    var QuestionPane = Widget.extend({
        ...
        showList: function() {
            ...
            // if (!pane._dataInitialized) {
                $.get(pane.get('plugin').api.init, {courseId:toolbar.get('courseId'), lessonId:toolbar.get('lessonId'),center:center}, function(html) {
        ...
        expandForm: function() {
            if($('#question-plugin-loginbox').length > 0){
                $('#question-plugin-loginbox').toggle();
                return ;
            }
            var pane = this,
