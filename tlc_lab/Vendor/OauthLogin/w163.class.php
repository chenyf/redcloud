<?php
require_once '163/config.php';
require_once('oauth.class.php');
require_once '163/weibooauth.class.php';
/**
 * 网易微博操作类 
 * 
 * @package sae 
...
class w163
{
	...
	{
		$this->__akey = W163_AKEY;
		$this->__skey = W163_SKEY;
	}
	public function getUrl()
	{
		...
		if (is_null($call_back)) {
			$call_back = U('career/Oauth/callback163');
		}

		$o = new W163OAuth( $this->__akey , $this->__skey  );
		$keys = $o->getRequestToken();
		$this->loginUrl = $o->getAuthorizeURL( $keys['oauth_token'] ,$call_back);
		$_SESSION['w163']['keys'] = $keys;
		return $this->loginUrl;
	}
	public function doClient($opt)
	{
		$oauth_token = ( $opt['oauth_token'] )? $opt['oauth_token']:$_SESSION['w163']['keys']['oauth_token'];
		$oauth_token_secret = ( $opt['oauth_token_secret'] )? $opt['oauth_token_secret']:$_SESSION['w163']['keys']['oauth_token_secret'];
		$this->oauth = new W163OAuth( $this->__akey , $this->__skey , $oauth_token , $oauth_token_secret);
		return $this->oauth;
	}
	//验证用户
	function checkUser(){
		$this->doClient();
		$access_token = $this->oauth->getAccessToken($_REQUEST['oauth_token']) ;
		$_SESSION['w163']['access_token'] = $access_token;
		$_SESSION['open_platform_type'] = 'w163';
	}
	//用户资料
	function userInfo(){
		$this->doClient($_SESSION['w163']['access_token']);
		$me = $this->verify_credentials();
		$user['id']          = $me['id'];
		$user['uname']       = $me['name'];
		$user['province']    = '';
		$user['city']        = '';
     ...
     * @return array 
     */ 
	function update( $text,$source ="<a href=\"http://www.uplook.cn\">尚观知识库</a>" )
	{
     ...
     * @return array 
     */ 
	function upload( $text, $pic_path,$source ="<a href=\"http://www.uplook.cn\">尚观知识库</a>"  )
	{
		$param = array();
		$param['pic'] = "@".$pic_path;
