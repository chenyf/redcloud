...
class ReviewServiceModel extends BaseModel{
    
    protected $currentDb = CENTER;
    
    private function getReviewDao(){
    	return $this->createDao("Course.Review");
    }
    ...
    public function saveReview($fields){
        ...
        if (empty($course)) {
            E("课程(#{$fields['courseId']})不存在，评价失败！");
        }
        $user = $this->getUserService()->getUser($fields['userId']);
        if (empty($user)) {
            return E("用户(#{$fields['userId']})不存在,评价失败!");
        }
