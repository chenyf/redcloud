...
class KeyApplierModel extends BaseModel
{
    ...
    public function applyKey($user, $edition = 'opensource', $source = 'apply')
    {
        ...
        $params['siteName'] = empty($site['name']) ? 'redcloud网络课程' : $site['name'];
        ...
        $url = empty($setting['cloud_api_server']) ? 'http://api.redcloud.net' : rtrim($setting['cloud_api_server'], '/');
        $url = $url . '/v1/keys';
    ...
    protected function postRequest($url, $params)
    {
        ...
        curl_setopt($curl, CURLOPT_USERAGENT, 'redcloud Install Client 1.0');
